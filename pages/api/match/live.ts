...
export default async function handler(
  ...
  if (!query.summonerName || !query.region)
    return res.status(400).json({ code: "LJ-001", error: "Missing query" });
  ...
  if (account.status?.status_code === 404)
    return res
      .status(404)
      .json({ code: "LJ-002", error: "Summoner not found" });

  const match = await getLiveMatch(account.id, query.region as Region);

  if (match.status?.status_code === 404)
    return res.status(404).json({ code: "LJ-003", error: "Match not found" });
  ...
  return res.status(200).json({ account, match, champions });
}
